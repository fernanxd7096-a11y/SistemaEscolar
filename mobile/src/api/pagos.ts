...
import type { MetodoPago, Pago, Paginacion, ResumenMetodoPago } from '../tipos';
...
export const obtenerPago = async (id: number): Promise<Pago> => {
  const { data } = await cliente.get(`/pagos/${id}`);
  return data;
};
